...
const ARTE_AUTH = 'Bearer YTEwZWE3M2UxMTVmYmRjZmE0YTdmNjA4ZTI2NDczZDU3YjdjYmVmMmRmNGFjOTM3M2RhNTM5ZjIxYmI3NTc1Zg';

function parseArteItem(item: any): MultimediaItem | null {
...
export async function getHome(cb: (res: Result<Record<string, MultimediaItem[]>>) => void) {
  try {
    ...
    for (const zone of zones) {
      ...
      const items = rawData.map(parseArteItem).filter((i: any): i is MultimediaItem => i !== null);
...
export async function load(url: string, cb: (res: Result<MultimediaItem>) => void) {
  try {
    ...
    // Check for episode links or single stream
...
export async function loadStreams(url: string, cb: (res: Result<StreamResult[]>) => void) {
  try {
    ...
    // Pattern 1: Akamaized HLS master playlist
